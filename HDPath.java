...
/**
 * HD Key derivation path. {@code HDPath} can be used to represent a full path
 * or a relative path. The {@code hasPrivateKey} {@code boolean} is used for
 * rendering to {@code String} but (at present) not much else. It defaults to
 * {@code false} which is the preferred setting for a relative path.
 * <p>
 * {@code HDPath} is immutable and uses the {@code Collections.UnmodifiableList}
 * type internally.
 * <p>
 * It implements {@code java.util.List<ChildNumber>} to ease migration from the
 * previous Guava {@code ImmutableList<ChildNumber>}. It should be a minor
 * breaking change to replace {@code ImmutableList<ChildNumber>} with
 * {@code List<ChildNumber>} where necessary in your code. Although it is
 * recommended to use the {@code HDPath} type for clarity and for access to
 * {@code HDPath}-specific functionality.
 * <p>
 * Take note of the overloaded factory methods {@link HDPath#M()} and
 * {@link HDPath#m()}. These can be used to very concisely create HDPath objects
 * (especially when statically imported.)
 */
public class HDPath extends AbstractList<ChildNumber> {
    ...
    /**
     * Create an HDPath from a path string. The path string is a human-friendly
     * representation of the deterministic path. For example:
     *
     * "44H / 0H / 0H / 1 / 1"
     *
