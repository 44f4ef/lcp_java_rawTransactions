...
public class Sha256Hash implements Serializable, Comparable<Sha256Hash> {
    ...
    /**
     * Creates a new instance that wraps the given hash value (represented as a hex
     * string).
     *
     * @param hexString a hash value represented as a hex string
     * @return a new instance
     * @throws IllegalArgumentException if the given string is not a valid hex
     *                                  string, or if it does not represent exactly
     *                                  32 bytes
     */
    ...
    /**
     * Creates a new instance that wraps the given hash value, but with byte order
     * reversed.
     *
     * @param rawHashBytes the raw hash bytes to wrap
     * @return a new instance
    ...
    /**
     * Creates a new instance containing the calculated (one-time) hash of the given
     * bytes.
     *
     ...
     * file's contents.
     *
     * The file contents are read fully into memory, so this method should only be
     * used with small files.
     *
     ...
     *
     * This is a convenience method which wraps the checked exception that can never
     * occur with a RuntimeException.
     *
    ...
    /**
     * Calculates the SHA-256 hash of the given bytes, and then hashes the resulting
     * hash again.
     *
     * @param input the bytes to hash
    ...
    /**
     * Calculates the hash of hash on the given chunks of bytes. This is equivalent
     * to concatenating the two chunks and then passing the result to
     * {@link #hashTwice(byte[])}.
     */
     ...
     * concatenating the two ranges and then passing the result to
     * {@link #hashTwice(byte[])}.
     */
     ...
     * to be a suitable hash code even for blocks, where the goal is to try and get
     * the first bytes to be zeros (i.e. the value as a big integer lower than the
     * target value).
     */
    @Override
    public int hashCode() {
