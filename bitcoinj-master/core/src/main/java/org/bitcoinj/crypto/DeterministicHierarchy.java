...
/**
 * <p>
 * A DeterministicHierarchy calculates and keeps a whole tree (hierarchy) of
 * keys originating from a single root key. This implements part of the BIP 32
 * specification. A deterministic key tree is useful because Bitcoin's privacy
 * system require new keys to be created for each transaction, but managing all
 * these keys quickly becomes unwieldy. In particular it becomes hard to back up
 * and distribute them. By having a way to derive random-looking but
 * deterministic keys we can make wallet backup simpler and gain the ability to
 * hand out {@link DeterministicKey}s to other people who can then create new
 * addresses on the fly, without having to contact us.
 * </p>
 *
 * <p>
 * The hierarchy is started from a single root key, and a location in the tree
 * is given by a path which is a list of {@link ChildNumber}s.
 * </p>
 */
public class DeterministicHierarchy {
    ...
    /**
     * Constructs a new hierarchy rooted at the given key. Note that this does not
     * have to be the top of the tree. You can construct a DeterministicHierarchy
     * for a subtree of a larger tree that you may not own.
     */
    ...
    /**
     * Inserts a key into the hierarchy. Used during deserialization: you normally
     * don't need this. Keys must be inserted in order.
     */
    public final void putKey(DeterministicKey key) {
        HDPath path = key.getPath();
        // Update our tracking of what the next child in each branch of the tree should
        // be. Just assume that keys are
     ...
     * @param create       whether the key corresponding to path should be created
     *                     (with any necessary ancestors) if it doesn't exist
     *                     already
     * @return next newly created key using the child derivation function
     * @throws IllegalArgumentException if create is false and the path was not
     *                                  found.
     */
    public DeterministicKey get(List<ChildNumber> path, boolean relativePath, boolean create) {
        HDPath inputPath = HDPath.M(path);
        HDPath absolutePath = relativePath ? rootPath.extend(path) : inputPath;
    ...
    /**
     * Extends the tree by calculating the next key that hangs off the given parent
     * path. For example, if you pass a path of 1/2 here and there are already keys
     * 1/2/1 and 1/2/2 then it will derive 1/2/3.
     *
     * @param parentPath        the path to the parent
     * @param relative          whether the path is relative to the root path
     * @param createParent      whether the parent corresponding to path should be
     *                          created (with any necessary ancestors) if it doesn't
     *                          exist already
     * @param privateDerivation whether to use private or public derivation
     * @return next newly created key using the child derivation function
     * @throws IllegalArgumentException if the parent doesn't exist and createParent
     *                                  is false.
     */
    public DeterministicKey deriveNextChild(List<ChildNumber> parentPath, boolean relative, boolean createParent,
    ...
    private ChildNumber getNextChildNumberToDerive(HDPath path, boolean privateDerivation) {
        ...
        ChildNumber nextChildNumber = new ChildNumber(lastChildNumber != null ? lastChildNumber.num() + 1 : 0,
                privateDerivation);
    ...
    /**
     * Extends the tree by calculating the requested child for the given path. For
     * example, to get the key at position 1/2/3 you would pass 1/2 as the parent
     * path and 3 as the child number.
     *
     * @param parentPath   the path to the parent
     * @param relative     whether the path is relative to the root path
     * @param createParent whether the parent corresponding to path should be
     ...
     *                                  is false.
     */
    public DeterministicKey deriveChild(List<ChildNumber> parentPath, boolean relative, boolean createParent,
    ...
    /**
     * Returns the root key that the {@link DeterministicHierarchy} was created
     * with.
     */
